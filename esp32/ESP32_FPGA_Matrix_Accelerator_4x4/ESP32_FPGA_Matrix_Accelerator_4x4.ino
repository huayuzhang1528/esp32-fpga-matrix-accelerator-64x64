...
// The remapped high-speed interface is hardware-validated at 20 MHz. The
// speed 0 command remains available as a conservative 100 kHz fallback.
uint32_t spiFrequencyHz = 20000000;
...
void fpgaLoadFourRowsOfA(const int8_t* fourRows, int n) {
    sendCommand(
        CMD_LOAD_A_BLOCK,
        reinterpret_cast<const uint8_t*>(fourRows),
...
void fpgaReadTile(int32_t tile[16]) {
    uint8_t zeros[64] = {};
    uint8_t bytes[64] = {};
...
// B must already be resident in the FPGA. A is streamed four rows at a time;
// each 4x4 C tile is read immediately, so the FPGA never needs a full C buffer.
bool fpgaRunWithResidentB(const int8_t* matrixA, int32_t* matrixC, int n) {
    if (!validSize(n))
        return false;

    for (int rowBase = 0; rowBase < n; rowBase += 4) {
        fpgaLoadFourRowsOfA(&matrixA[rowBase * n], n);
...
void runCommand(int n) {
    if (!validSize(n)) {
        Serial.println("Size must be a multiple of 4 from 4 through 32.");
        return;
    }
...
void printHelp() {
    ...
    Serial.println("  test N    CPU/FPGA correctness test; N is a multiple of 4, max 32");
    ...
    Serial.println("Default validated setting: 20 MHz. Run test 4/8/16/32 after programming.");
}
...
void setup() {
    ...
    delay(500);
    Serial.println("ESP32 + FPGA 4x4 matrix accelerator ready.");
